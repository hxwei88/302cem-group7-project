function append_stock_display(result) {
    ...
    result.forEach(function (item, index, arr) {
        ...
        html += '<div class="col-3 py-2 mb-3">' +
                ...
                '<img class="card-img-top" style="height: 325px; margin: auto; border-radius: 2%" src="' + image + '" alt="' + image + '">' +
                '<div class="card-body" style = "text-align:center;">' +
                '<h5 class="card-title">' + item.name + '</h5>' +
                '<p>' + item.isbn + '</p>' +
                '</div>' +
                '<button type="button" style="height: 50px; width: 80%; margin:auto; font-size:115%; margin-bottom:20px;" class="btn btn-primary" value=' + item.isbn + ' onclick ="load_detail_page(this.value)">Details</button>' +
                '</div>' +
                '</div>';
    })
